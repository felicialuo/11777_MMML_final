...
class CLAPWrapper():
    ...
    def _get_audio_embeddings(self, preprocessed_audio, return_all: bool = True):
        r"""Load preprocessed audio and return a audio embeddings"""
        # with torch.no_grad():
        #     preprocessed_audio = preprocessed_audio.reshape(
        #         preprocessed_audio.shape[0], preprocessed_audio.shape[2])
        #     #Append [0] the audio emebdding, [1] has output class probabilities, [2] has temporal information of audio embedding
        #     return self.clap.audio_encoder(preprocessed_audio)[0 if not return_temporal else 2]
        
        preprocessed_audio = preprocessed_audio.reshape(
            preprocessed_audio.shape[0], preprocessed_audio.shape[2])
        #Append [0] the audio emebdding, [1] has output class probabilities, [2] has temporal information of audio embedding
        return self.clap.audio_encoder(preprocessed_audio)[0 if not return_all else 2]
