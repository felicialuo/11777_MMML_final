...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        if self.config.enable_tscam:
            ...
            # get full temporal latent out
            full_latent_output = x.flatten(2).transpose(1, 2)
            ...
            if self.config.loss_type == "clip_ce":
                output_dict = {
                    ...
                    'full_latent': full_latent_output
                }
            else:
                output_dict = {
                    'framewise_output': fpx, # already sigmoided
        ...
        if infer_mode:
            ...
            if x.shape[2] > self.freq_ratio * self.spec_size:
                if self.training:
                    ...
                    full_latent = torch.zeros_like(output_dicts[0]["full_latent"]).float().to(x.device)
                    for d in output_dicts:
                        clipwise_output += d["clipwise_output"]
                        framewise_output += d["framewise_output"]
                        latent_output += d["latent_output"]
                        full_latent += d['full_latent']
                    clipwise_output  = clipwise_output / len(output_dicts)
                    framewise_output = framewise_output / len(output_dicts)
